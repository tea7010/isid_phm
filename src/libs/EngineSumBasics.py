            ...
            for regime_i in [1, 2, 3, 4, 5, 6]:
                ...
                for col_i in TARGET_COLS:
                    colname = 'Regime_%s_%s_%s' % (
                        regime_i, col_i, 'mean')
                    summarize_df.loc[eg_i, colname] = regime_df[col_i].mean()
                    ...
                    colname = 'Regime_%s_%s_%s' % (
                        regime_i, col_i, 'max')
                    summarize_df.loc[eg_i, colname] = regime_df[col_i].max()

                    colname = 'Regime_%s_%s_%s' % (
                        regime_i, col_i, 'min')
                    summarize_df.loc[eg_i, colname] = regime_df[col_i].min()
        return summarize_df
